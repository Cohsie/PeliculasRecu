  ...
  async doLogin(username: string, password: string){
    ...
    try {
      ...
      if (RESPONSE.data?.token) {//Si se ha obtenido el token sesion
        ...
        });//Para refrescar los datos del commonService aunque no parece útil.
        ...
        } else{//Hacer el procedimiento de redirección solo si el usuario tiene una api_movies válida. Si tiene una pero errónea dará error
          const popup = window.open('https://www.themoviedb.org/logout', 'ventanaLogOut', 'width=500,height=600');

          const checkClosed = setInterval(() => {
            if (popup?.closed) {
              clearInterval(checkClosed);//Terminar el intervalo de ejecución para dejar de comprobar si la ventana está cerrada
              ...
              this.authService.getRequestToken().subscribe({
                ...
                next:(tokenResponse) => {
                  ...
                  //Redirección del usuario para iniciar sesión, validar el token y volver a la aplicación

                  window.location.href = redirectURL;//Uso de la constante para hacer la redirección

                },
                error: (error) => {//Se llega aquí por problemas en la conexion con el back
                  console.error('Error al obtener el request token: ', error);//Si la api_movies está mal da error
                  this.snackBar.open('Error al obtener el request token', 'Cerrar', { duration: 5000 });
                  this.authService.doLogout();//Si se quita esto se puede comprobar el error
                }
              });
            }
          }, 500);
